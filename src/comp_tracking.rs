...
use usiem::{
    ...
    prelude::{
        task::{SiemTask, SiemTaskResult},
        SiemCommandResponse, SiemDataset,
    },
};
use usiem::{
...
impl ComponentTracking {
    ...
    pub fn run_component_generic(
        ...
        let new_comp = (*comp).duplicate();
        let comp_name = LogString::Owned(new_comp.name().to_string());
        let thread_info = Self::run_component(component_id, new_comp, &build_info, state_storage);
    ...
    fn run_component(
        ...
        let thread_join = std::thread::spawn(move || {
            ...
            params.insert(
                ...
                LogString::Owned(component_id.to_string()),
            );
            res
        });
    ...
    pub fn channel_of_component(&self, comp_id: u64) -> Option<&Sender<SiemMessage>> {
        self.thread_map
            .get(&comp_id)
            .and_then(|(_handler, channel)| Some(channel))
    }
    ...
    pub fn route_message(
        ...
        match msg {
            ...
            SiemMessage::Response(header, response) => self.route_response(header, response),
    ...
    fn notify_dataset(&mut self, dataset: SiemDataset, build_info: &ComponentBuildingInfo) {
        let component_names = match build_info
            .store
            .get_components_for_dataset(&dataset.dataset_type())
        {
            Some(v) => v,
            None => return,
        };
    ...
    fn route_response(
        ...
        if (now - command_info.timestamp) > timeout {
            ...
            self.command_tracking
                .retain(|_k, v| (now - v.timestamp) < timeout);
            return Ok(());
        }
        ...
        let _ = channel.try_send(SiemMessage::Response(
            SiemCommandHeader {
                user: header.user.clone(),
                comp_id: header.comp_id,
                comm_id: command_info.original_msg_id,
            },
            response,
        ));
        Ok(())
    ...
    fn route_command(
        ...
        for comp_name in components_for_command {
            ...
            let _ = channel.try_send(SiemMessage::Command(
                SiemCommandHeader {
                    user: header.user.clone(),
                    comp_id: header.comp_id,
                    comm_id: new_command_id,
                },
                command.clone(),
            ));
        }
    ...
    pub fn update_dataset(&self, dataset: SiemDataset) {
        for (_id, (_component, sender)) in &self.thread_map {
            let _ = sender.send_timeout(
                SiemMessage::Dataset(dataset.clone()),
    ...
    fn process_task(
        ...
        let task_definitions = match build_info
            .store
            .get_definitions_for_task(&task.data.class())
        {
            Some(v) => v,
            None => return Ok(()),
        };
        ...
        for task_def in task_definitions {
            ...
            match async_std::task::Builder::new()
                .name(format!("Task-{}-{}", task.data.class(), now))
                .spawn(new_task)
            {
                Ok(v) => {
                    let end_task = (chrono::Utc::now()
                        + Duration::milliseconds(task_def.max_duration() as i64))
                    .timestamp_millis();
