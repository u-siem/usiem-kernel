...
use comp_tracking::{ComponentTracking, ComponentBuildingInfo};
use metrics::*;
use usiem::crossbeam_channel::{ TryRecvError};
use usiem::crossbeam_channel;
use usiem::prelude::{CommandResult, SiemCommandResponse, SiemCommandHeader};
...
impl SiemBasicKernel {
    pub fn new(channel_size: usize, max_threads: usize, command_timeout: i64) -> SiemBasicKernel {
        return SiemBasicKernel {
            channels : ComponentChannels::new(channel_size, max_threads as f64),
            components : SiemComponentStore::default(),
            command_response_timeout: command_timeout,
            metrics : generate_kernel_metrics(),
            max_threads,
    ...
    pub fn run(&mut self) {
        ...
        match self.dataset_manager.take() {
            Some(mut comp) => {
                let id = comp.id();
                let msngr = KernelMessager::new(id, comp.name().to_string(), self.channels.kernel_channel.1.clone());
                comp.set_kernel_sender(msngr.clone());
                let local_channel = comp.local_channel();
                let thread_join = thread::spawn(move || {
                    usiem::logging::initialize_component_logger(msngr);
                    comp.run()
                });
                component_tracking.register_external_component(id, (thread_join, local_channel));
            }
            None => {
        ...
        loop {
            ...
            #[cfg(feature = "metrics")]
            {
                ...
                TOTAL_MESSAGES_PROCESSED.store(total_messages_processed, std::sync::atomic::Ordering::Relaxed);
            }
                ...
                match self.channels.kernel_channel.0.try_recv() {
                    Ok(msg) => {
                        ...
                        if self.is_message_for_kernel(&msg) {
                            match self.process_message_for_kernel(msg, &mut component_tracking, &mut build_info) {
                                Ok(_) => {},
                                Err(v) => {
                                    println!("{}",v);
                                    return;
    ...
    fn process_message_for_kernel(&mut self, message : SiemMessage, component_tracking : &mut ComponentTracking, build_info : &mut ComponentBuildingInfo) -> Result<(), &'static str>{
        match message {
                ...
                match command {
                    ...
                    SiemCommandCall::GET_TASK_RESULT(task_id) => {
                        ...
                        let result = if let Some(result) = result {
                            ...
                            CommandResult::Err(usiem::prelude::CommandError::NotFound(LogString::Borrowed("Task has not finished")))
                        };
                        component_tracking.send_message_to_component(header.comp_id, SiemMessage::Response(SiemCommandHeader {
    ...
    fn is_message_for_kernel(&self, message : &SiemMessage) -> bool {
        match message {
            SiemMessage::Command(_header, command) => {
                match command {
    ...
    fn scale_components(&self, tracking : &mut ComponentTracking, names : &ComponentNames, build_info : &ComponentBuildingInfo) {
...
mod tests {
    ...
    fn test_kernel_instance() {
        ...
        if let SiemMetric::Counter(val) = metrics.get("total_messages_processed_by_kernel").unwrap() {
            ...
            assert!(val.load(Ordering::Relaxed) >= 20i64);
        }else{ unreachable!("Must be a counter")}
